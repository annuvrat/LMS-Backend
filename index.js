...
app.post('/attendance-correction', authenticateJWT, authorizeEmployee, async (req, res) => {
  ...
  if (!empl_code || !absentDate || !reason) {
    return res.status(400).json({ error: 'EMPL_CODE, absent date, and reason are required.' });
  }

  try {
    const pool = await sql.connect(config1); // Connect to the database using config2

    // Check if an attendance correction for the same date already exists
    const existingRequestResult = await pool.request()
      .input('empl_code', sql.VarChar, empl_code)
      .input('absentDate', sql.Date, absentDate)
      .query(`
        SELECT * FROM [dbo].[AttendanceCorrectionRequests] 
        WHERE [EMPL_CODE] = @empl_code AND [absentDate] = @absentDate
      `);

    if (existingRequestResult.recordset.length > 0) {
      return res.status(400).json({ error: 'Attendance correction request already exists for the specified date.' });
    }

    // Insert the attendance correction request into the database
    await pool.request()
      .input('empl_code', sql.VarChar, empl_code)
      .input('absentDate', sql.Date, absentDate)
      .input('reason', sql.NVarChar, reason)
      .input('status', sql.VarChar, 'Pending')
      .query(`
        INSERT INTO [dbo].[AttendanceCorrectionRequests]
        ([EMPL_CODE], [absentDate], [reason], [status])
        VALUES (@empl_code, @absentDate, @reason, @status)
      `);

    res.status(201).json({ message: 'Attendance correction request submitted successfully.' });
  } catch (err) {
    console.error('Error submitting attendance correction request:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
app.post('/approve-attendance-correction/:id', authenticateJWT, authorizeManagerOrHR, async (req, res) => {
  ...
  try {
    const pool = await sql.connect(config1); // Connect to the database

    // Update the status of the attendance correction request
    await pool.request()
      .input('id', sql.Int, id)
      .input('status', sql.VarChar, status) // e.g., 'Approved'
      .query(`
        UPDATE [dbo].[AttendanceCorrectionRequests]
        SET [status] = @status
        WHERE [ID] = @id
      `);

    // Change attendance status from 'Absent' to 'Present' for the specified date
    const absentDateQuery = await pool.request()
      .input('id', sql.Int, id)
      .query(`
        SELECT absentDate FROM [dbo].[AttendanceCorrectionRequests]
        WHERE [ID] = @id
      `);

    if (absentDateQuery.recordset.length > 0) {
      const absentDate = absentDateQuery.recordset[0].absentDate;
      ...
      await pool.request()
        ...
        .query(`
          ...
          SET [STATUS] = 'Present'
          WHERE [EMPL_CODE] = @empl_code AND [ATTENDANCE_DATE] = @absentDate
        `);
    }

    res.status(200).json({ message: 'Attendance correction request approved successfully.' });
  } catch (err) {
    console.error('Error approving attendance correction request:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
